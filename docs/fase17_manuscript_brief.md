...
- Figures must be supplied as separate files with logical names; tables must be editable text, not images. The generated review PDF also embeds the main figures and main-table previews at the end so reviewers can evaluate the display items in a single PDF.
