...
- Public repository, final code release, and DOI: resolved for this draft. The public repository URL is https://github.com/vicenzoscavino1999/surfaceome-gastric-cancer, with final code release tag `v0.1.1`. The frozen reproducibility data package is archived on Zenodo at https://zenodo.org/records/20498705 with DOI `10.5281/zenodo.20498705`, covering the frozen input bundle and checksum/provenance data package.
...
- Reviewer reproducibility is locally auditable through `docs/reproducibility_reviewer_guide.md` and `python scripts/run_reproducibility_checks.py`. `docs/source_acquisition_policy.md` records that API/manual captures are frozen inputs rather than a live-redownload claim. The final public tag `v0.1.1` passed GitHub Actions small CI/container build and a local clean public-clone frozen-raw forced rerun; DOI-bound data-package archival is complete for `v0.1.0-rc4`.
