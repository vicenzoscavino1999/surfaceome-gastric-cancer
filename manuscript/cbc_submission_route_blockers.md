...
- Public repository, final code release, and DOI: resolved for this draft. The public repository URL is https://github.com/vicenzoscavino1999/surfaceome-gastric-cancer, with final code release tag `v0.1.3`. The frozen reproducibility data package is archived on Zenodo at https://zenodo.org/records/20498705 with DOI `10.5281/zenodo.20498705`, covering the frozen input bundle and checksum/provenance data package.
...
- Reviewer reproducibility is locally auditable through `docs/reproducibility_reviewer_guide.md` and `python scripts/run_reproducibility_checks.py`, which auto-detects a bare clone and runs a no-raw smoke audit when the frozen data bundle is absent. `docs/source_acquisition_policy.md` records that API/manual captures are frozen inputs rather than a live-redownload claim. The prior public tags `v0.1.1` and `v0.1.2` passed GitHub Actions CI and container build (the `v0.1.2` push CI passed on Linux, including the no-raw smoke audit); `v0.1.3` finalizes the manuscript and embedded display items with no data, score, or ranking change (the frozen ranking SHA256 is unchanged), and its post-public-tag clone/container audit is to be recorded after the tag is pushed. DOI-bound data-package archival is complete for `v0.1.0-rc4`.
