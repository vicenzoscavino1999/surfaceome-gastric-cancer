...
- Public repository and DOI: the public repository URL is https://github.com/vicenzoscavino1999/surfaceome-gastric-cancer. Before submission, archive the release/data package with Zenodo, OSF, Figshare, or an equivalent repository and insert the DOI in the manuscript and cover letter. The DOI must cover the frozen input bundle or an equivalent checksum/provenance data package for inputs that cannot be redistributed.
...
- Reviewer reproducibility is locally auditable through `docs/reproducibility_reviewer_guide.md` and `python scripts/run_reproducibility_checks.py`. `docs/source_acquisition_policy.md` records that API/manual captures are frozen inputs rather than a live-redownload claim. Clean-clone/container audit remains pending until the public tag is frozen; DOI-bound data-package audit remains pending until the archival package exists.
...
- Data/code availability: remove remaining draft wording once the archival DOI is available.
