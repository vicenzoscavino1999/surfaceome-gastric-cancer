...
- Public repository, final code release, and DOI: resolved for this draft. The public repository URL is https://github.com/vicenzoscavino1999/surfaceome-gastric-cancer, with final code release tag `v0.1.7`. The frozen reproducibility data package is archived on Zenodo at https://zenodo.org/records/20498705 with DOI `10.5281/zenodo.20498705`, covering the frozen input bundle and checksum/provenance data package.
...
- The current generated profile follows an Elsevier `elsarticle` review-profile author-year manuscript for Computational Biology and Chemistry. Do not hand-edit `manuscript/latex/cbc_manuscript.tex`.
...
- Reviewer reproducibility is locally auditable through `docs/reproducibility_reviewer_guide.md` and `python scripts/run_reproducibility_checks.py`, which auto-detects a bare clone and runs a no-raw smoke audit when the frozen data bundle is absent. `docs/source_acquisition_policy.md` records that API/manual captures are frozen inputs rather than a live-redownload claim. Release CI and container status are tracked on GitHub Actions for the corresponding public tag. `v0.1.7` places the manuscript figures and tables inline near their first mention with no data, score, or ranking change (the frozen ranking SHA256 is unchanged). DOI-bound data-package archival is complete for `v0.1.0-rc4`.
...
- Graphical abstract: exported as a separate TIFF meeting Elsevier size guidance; final user approval still required after any wording changes.
- Data/code availability: DOI and final release tag wording inserted; re-check after any manuscript-title edits.
