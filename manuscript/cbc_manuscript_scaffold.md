...
### 1.2 Clinical and biological motivation
...
The novelty boundary is therefore deliberately narrow. Recent literature already includes pan-cancer surfaceome resources, pan-cancer ADC target discovery, tumor-specificity modeling, gastric clinical-trial target prioritization, gastric proteogenomic vulnerability atlases, and single-target or subtype-focused gastric/GEJ/GSRCC ADC discovery studies [3,6-8,19,20,31-39]. The present contribution is not a first gastric target-discovery claim; it is a release-grade, uncertainty-aware framework for auditing how heterogeneous public surfaceome evidence is combined and bounded.

### 1.3 Contribution

This study contributes an uncertainty-aware, reproducible framework for transparent gastric cancer surface-target hypothesis generation from public evidence. The framework makes the limits of bulk RNA, protein/localization coverage, topology, normal-tissue risk, TME admixture, isoform ambiguity, and external benchmarks part of the result rather than compressing them into a single ranked list. As a concrete methodological audit, it quantifies a GPI-anchor evidence-routing gap and implements a source-wide correction at universe construction before scoring. It also shows that external gastric-atlas overlap exceeds a simple random-draw null but not a matched null, making benchmark consistency without independent validation an intended finding rather than a late caveat. Coarse Tier 1/2 nominations are downstream, experimentally testable hypotheses used to illustrate the framework's boundaries.
...
### 3.1 Framework behavior and external consistency
...
The TCSA and Wang comparisons address different external-reference questions. TCSA provides a pan-cancer surfaceome-score baseline: the moderate rank correlation and low top20 identity show that the workflow is aligned with established surfaceome evidence but not reducible to it. Wang 2026 is best interpreted as a gastric-specific consistency check rather than an independent validation claim. Other contemporary gastric proteogenomic work provides therapeutic-vulnerability context but is not treated here as a validation label [34]. The overlap with Wang exceeds a simple random draw but not a matched null, which matters because both resources enrich for highly expressed and well-annotated surface proteins. Taken together, the external checks support plausibility and benchmark consistency, but not independent validation of the ranking or any individual nomination. CD9 and LSR remain divergent nominations, glyco-outlier corroboration is selective, and the curated Wang Figure 7H single-cell panel does not directly plot most of the specific nominees.
...
This study provides a reproducible framework for uncertainty-aware gastric cancer surface-target prioritization from public evidence. Its central product is not a target list alone, but a transparent record of what heterogeneous bulk RNA, protein/localization, topology, normal-risk, surfaceome, and benchmark data can and cannot establish.
...
Analysis code is in `scripts/`, `src/`, `workflow/Snakefile`, and the configuration files in `config/`. The public code repository is https://github.com/vicenzoscavino1999/surfaceome-gastric-cancer, with final code release tag `v0.1.1`. The frozen reproducibility data package for release candidate `v0.1.0-rc4` is archived on Zenodo at https://zenodo.org/records/20498705 with DOI `10.5281/zenodo.20498705`. The repository includes smoke tests, unit tests, manuscript checks, Snakemake dry-run checks, GitHub Actions small CI, and manual full-data release-audit hooks documented in the README and reproducibility guide. This DOI covers the frozen inputs and checksum/provenance data package required for the Computational Biology and Chemistry research-data route because the journal asks authors to deposit research data in a relevant repository and cite or link the dataset in the article.
