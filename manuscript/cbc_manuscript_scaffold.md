...
Analysis code is in `scripts/`, `src/`, `workflow/Snakefile`, and the configuration files in `config/`. The repository includes smoke tests, unit tests, manuscript checks, Snakemake dry-run checks, GitHub Actions small CI, and manual full-data release-audit hooks documented in the README and reproducibility guide. The submission version will include a public repository URL and archival DOI after the release package is archived; the DOI must cover the frozen inputs or an equivalent checksum/provenance data package. This is required for the Computational Biology and Chemistry research-data route because the journal asks authors to deposit research data in a relevant repository and cite or link the dataset in the article; those items remain pre-submission blockers in this draft.
